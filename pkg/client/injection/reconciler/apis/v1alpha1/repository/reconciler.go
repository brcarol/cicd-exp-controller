...
import (
	versioned "cicd-exp-controller/pkg/client/clientset/versioned"
	apisv1alpha1 "cicd-exp-controller/pkg/client/listers/apis/v1alpha1"
	context "context"
	json "encoding/json"
	fmt "fmt"
...
// controller reconciling v1alpha1.Repository.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.Repository. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.Repository) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.Repository.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.Repository. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.Repository) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.Repository if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.Repository.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.Repository) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.Repository) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.Repository resources.
type reconcilerImpl struct {
	...
	Lister apisv1alpha1.RepositoryLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister apisv1alpha1.RepositoryLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Repositories(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.Repository, desired *v1alpha1.Repository) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EcrV1alpha1().Repositories(desired.Namespace)
		...
		updater := r.Client.EcrV1alpha1().Repositories(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Repository, desiredFinalizers sets.Set[string]) (*v1alpha1.Repository, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.EcrV1alpha1().Repositories(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.Repository) (*v1alpha1.Repository, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.Repository, reconcileEvent reconciler.Event) (*v1alpha1.Repository, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
